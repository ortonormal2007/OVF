...
double right1(double y, double x){
	return (10 * x) - (2 * x * y);
}

double right2(double y, double x){
	return (2 * x * y) - (10 * y);
}
...
void rungekutta(double x, double y, double t, double tfinal, double step, double(*right1)(double, double), double(*right2)(double, double)){
	...
	double t_cur = t + step;
	while (t_cur <= tfinal){
		x_next = x + step * (0.25 * right1(y, x) + 0.75 * right1(y + ((2. / 3.) * step * right2(y, x)), x + ((2. / 3.) * step * right1(y, x))));
		y_next = y + step * (0.25 * right2(y, x) + 0.75 * right2(y + ((2. / 3.) * step * right2(y, x)), x + ((2. / 3.) * step * right1(y, x))));
...
int main()
{
	...
	cin >> tfinal;
	rungekutta(xinit, yinit, tinit, tfinal, 0.001, right1, right2);
